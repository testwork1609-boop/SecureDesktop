...
namespace SecureDesktop.Services
{
    public class PatternRecognitionService : IDisposable
    {
                    ...
                    try
                    {
                        ...
                        if (screen != null)
                        {
                            foreach (var pattern in _patterns)
                            {
                                ...
                                Rectangle result =
                                    FindPattern(screen, pattern);


                                if (result != Rectangle.Empty)
                                {
                                    bool changed =
                                        !_lastLocations.ContainsKey(pattern.Source.Id)
                                        ||
            ...
            CachedPattern pattern)
        {
            ...
            // najpierw sprawdzamy poprzednią pozycję
            if (_lastLocations.TryGetValue(
                pattern.Source.Id,
                out Rectangle last))
            {
                searchArea =
            ...
            unsafe
            {
                ...
                for(int y=0;
                    y < searchArea.Height-pattern.Height;
                    y+=3)
                {
                    for(int x=0;
                        x < searchArea.Width-pattern.Width;
                        x+=3)
                    {
        ...
        private unsafe double CompareFast(
            byte* screen,
            int stride,
            int x,
            int y,
